...
export function diffStates(
  fromCommit: string,
  toCommit: string,
  oldState: Record<string, Record<string, any>>,
  newState: Record<string, Record<string, any>>,
...
function extractItemName(parts: string[]): string {
...
function describeLayoutChange(type: string, parts: string[], name: string, oldVal: any, newVal: any): string {
  const prop = parts[parts.length - 1];
  
  if (type === "added") return `Added layer '${name}'`;
  if (type === "removed") return `Removed layer '${name}'`;
...
function describeStyleChange(type: string, parts: string[], name: string, oldVal: any, newVal: any): string {
  const prop = parts[parts.length - 1];
  
  if (type === "added") return `Added style for '${name}'`;
  if (type === "removed") return `Removed style for '${name}'`;
...
function describeTypographyChange(type: string, parts: string[], name: string, oldVal: any, newVal: any): string {
  const prop = parts[parts.length - 1];
  
  if (type === "added") return `Added text element '${name}'`;
  if (type === "removed") return `Removed text element '${name}'`;
...
function describeAnimationChange(type: string, parts: string[], name: string, oldVal: any, newVal: any): string {
  const prop = parts[parts.length - 1];
  
  if (type === "added") return `Added animation on '${name}'`;
  if (type === "removed") return `Removed animation on '${name}'`;
...
function describeAudioChange(type: string, parts: string[], name: string, oldVal: any, newVal: any): string {
  const prop = parts[parts.length - 1];
  
  if (type === "added") return `Added audio track '${name}'`;
  if (type === "removed") return `Removed audio track '${name}'`;
...
function describeGenericChange(type: string, path: string, oldVal: any, newVal: any): string {
  if (type === "added") return `Added ${path}`;
  if (type === "removed") return `Removed ${path}`;
