...
import type { Actor, AgentMetadata } from "../core/types.js";
...
program
  ...
  .action(async (name: string, options: { adapter: string }) => {
...
proposalCmd
  ...
  .action(async (options) => {
    ...
    const actor: Actor = options.agentName ? { type: "agent", name: options.agentName } : { type: "human", name: "User" };
    ...
    const proposal = await pm.createProposal(
      "proj_01",
...
proposalCmd
  ...
  .action((id) => {
    ...
    if (!p) return console.log(chalk.red("Not found"));
