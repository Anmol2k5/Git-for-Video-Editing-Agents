 ...
 */
export function normalizeResolveTimeline(timeline: ResolveTimeline): DomainFile[] {
 ...
 */
export function denormalizeToResolveTimeline(domainFiles: DomainFile[]): ResolveTimeline {
  const getFile = (domain: string) => domainFiles.find(f => f.domain === domain)?.data || {};
  ...
  const timeline: ResolveTimeline = {
    ...
    tracks: [],
    markers: Object.values(markers) as ResolveMarker[]
  };

  // We need to rebuild tracks from clips
  const trackMap = new Map<string, ResolveTrack>();

  const getOrCreateTrack = (trackId: string, trackName: string, type: "video"|"audio"|"subtitle") => {
  ...
  Object.entries(cuts).forEach(([clipId, cutData]: [string, any]) => {
    ...
    if (effects[clipId]) clip.effects = effects[clipId];
    if (color[clipId]) clip.colorGrade = color[clipId];

    track.clips.push(clip);
  });

  // Reassemble Audio
  Object.entries(audio).forEach(([clipId, audioData]: [string, any]) => {
  ...
  // Reassemble Captions
  Object.entries(captions).forEach(([clipId, capData]: [string, any]) => {
