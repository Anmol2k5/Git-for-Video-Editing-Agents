...
program
    ...
    if (options.live) {
        const meta = snapshot.domains.find((d) => d.domain === "metadata")?.data;
        const cuts = snapshot.domains.find((d) => d.domain === "cuts")?.data;
        const audio = snapshot.domains.find((d) => d.domain === "audio")?.data;
        const markers = snapshot.domains.find((d) => d.domain === "markers")?.data;
