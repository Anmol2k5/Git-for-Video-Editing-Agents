...
export class EditVCSEngine {
     ...
     */
    async init(name, adapter = "generic", domains) {
    ...
    async commit(message, actor) {
        ...
        writeFileSync(join(commitsDir, `${shortId}.json`), JSON.stringify(commitMeta, null, 2));
        await this.git.add(".editvcs/");
    ...
    async log(maxCount = 50) {
        ...
        return gitLog.all.map(entry => {
            ...
            let actor = { type: "human", name: entry.author_name };
            let domains = [];
    ...
    async importState(state) {
        ...
        if (adapter && adapter.importState) {
            const snapshot = await adapter.importState(state);
